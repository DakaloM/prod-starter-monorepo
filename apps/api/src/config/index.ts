import { env } from '@imax/serverkit';
...
const builder = {
  ...
  intergration: {
    sap: {
      user: env.get('SAP_USER').asString(),
      password: env.get('SAP_PASSWORD').asString(),
      baseUrl: env.get('SAP_BASE_URL').asString(),
      client: env.get('SAP_CLIENT').asString(),
    }
  },
  account: {
    verifySecret: env.get('VARIFY_SECRET').asString(),
  }
};
